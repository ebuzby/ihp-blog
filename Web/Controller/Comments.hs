...
import Web.Controller.Prelude (CommentsController(postId), Comment' (postsId))
...
instance Controller CommentsController where
    ...
    action CreateCommentAction = do
        ...
        comment
            ...
            |> ifValid \case
                Left comment -> do 
                    post <- fetch (get #postsId comment)
                    render NewView { .. } 
                Right comment -> do
                    comment <- comment |> createRecord
                    setSuccessMessage "Comment created"
                    redirectTo ShowPostAction { postId = get #postsId comment }
